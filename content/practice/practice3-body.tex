\underline{Exercise $4.2$}
\begin{itemize}

% Question 1
\item[1.] Given $A=\left[\begin{array}{cc}7 & -1 \\ 6 & 9\end{array}\right],  B=\left[\begin{array}{cc}0 & 4 \\ 3 & -2\end{array}\right], \text{ and }C=\left[\begin{array}{ll}8 & 3 \\ 6 & 1\end{array}\right]$, find: 
\begin{multicols}{2}
\begin{enumerate}\renewcommand{\labelenumi}{\alph{enumi})}
\item [(a)] $A+B$
\item [(b)] $C-A$
\item [(c)] $3A$
\item [(d)] $4B+2C$ 
\end{enumerate}
\end{multicols}

% Question 2
\item[2.] Given $A = \left[\begin{array}{cc}
2 & 8 \\
3 & 0 \\
5 & 1
\end{array}\right], B=\left[\begin{array}{cc} 2 & 0 \\ 3 & 8\end{array}\right], \text{ and }
C=\left[\begin{array}{cc} 7 & 2 \\ 6 & 3\end{array}\right]$:
\begin{enumerate}
	\item[(a)] Is $AB$ defined? Calculate $AB$. Can you calculate $BA$? Why?
	\item[(b)] Is $BC$ defined? Calculate $BC$. Is $CB$ defined? If so, calculate $CB$. Is it true that $BC=CB$. 
\end{enumerate}

% Question 4
\item[4.] Find the product matrices in the following (in each case, append beneath every matrix a dimension indicator): 
\begin{multicols}{2}
\begin{enumerate}\renewcommand{\labelenumi}{\alph{enumi})}
\item [(a)] $\left[\begin{array}{lll}0 & 2 & 0 \\ 3 & 0 & 4 \\ 2 & 3 & 0\end{array}\right]\left[\begin{array}{ll}8 & 0 \\ 0 & 1 \\ 3 & 5\end{array}\right]$
\item [(b)] $\left[\begin{array}{rrr}6 & 5 & -1 \\ 1 & 0 & 4\end{array}\right]\left[\begin{array}{rr}4 & -1 \\ 5 & 2 \\ 0 & 1\end{array}\right]$
\item [(c)] $\left[\begin{array}{rrr}3 & 5 & 0 \\ 4 & 2 & -7\end{array}\right]\left[\begin{array}{l}x \\ y \\ z\end{array}\right]$
\item [(d)] $\left[\begin{array}{lll}a & b & c\end{array}\right]\left[\begin{array}{ll}7 & 0 \\ 0 & 2 \\ 1 & 4\end{array}\right]$ 
\end{enumerate}
\end{multicols}
\end{itemize}

%%%%% Exercise $4.4$
