\underline{Exercise $4.4$}
\begin{itemize}

% Question 5
\item[5.]
\begin{itemize}
\item[(e)] Find (i) $C=A B$, and (ii) $D=B A$, if
\[
A=\left[\begin{array}{r}
-2 \\
4 \\
7
\end{array}\right] \quad B=\left[\begin{array}{lll}
3 & 6 & -2
\end{array}\right]
\]
\end{itemize}

% Question 7
\item[7.] If the matrix $A$ in Example 5 had all its four elements nonzero, would $x^T A x$ still give a weighted sum of squares? Would the associative law still apply? \\
\end{itemize}

%%%%% Exercise 4.5

\underline{Exercise $4.5$}
\begin{itemize}
% Question 1
\item[1.] Given $A=\left[\begin{array}{rrr}-1 & 5 & 7 \\ 0 & -2 & 4\end{array}\right], b=\left[\begin{array}{l}9 \\ 6 \\ 0\end{array}\right]$, and $x=\left[\begin{array}{l}x_1 \\ x_2\end{array}\right]$ : \\~\\
Calculate: (a) $AI \quad$ (b) $I A \quad$ (c) $I x \quad$ (d) $x^T I$ \\
Indicate the dimension of the identity matrix used in each case.

% Question 4
\item[4.] Show that the diagonal matrix
\[
\left[\begin{array}{cccc}
a_{11} & 0 & \cdots & 0 \\
0 & a_{22} & \cdots & 0 \\
\cdots  & \cdots & \cdots & \cdots  \\
0 & 0 & \cdots & a_{n n}
\end{array}\right]
\]
can be idempotent only if each diagonal element is either 1 or 0 . How many different numerical idempotent diagonal matrices of dimension $n \times n$ can be constructed altogether from such a matrix? \\
\end{itemize}

%%%%% Exercise 4.6

\underline{Exercise $4.6$}
\begin{itemize}
% Question 2
\item[2.] Given $A=\left[\begin{array}{rr}0 & 4 \\ -1 & 3\end{array}\right], B=\left[\begin{array}{rr}3 & -8 \\ 0 & 1\end{array}\right]$, and $C=\left[\begin{array}{lll}1 & 0 & 9 \\ 6 & 1 & 1\end{array}\right]$, verify that
\begin{enumerate}
\item[(a)] $(A+B)^T=A^T+B^T$
\item[(b)] $(A C)^T=C^T A^T$
\end{enumerate}

% Question 6
\item[6.]  Let \( A=I-X\left(X^T X\right)^{-1} X^T \).
\begin{enumerate}
\item[(a)] Must $A$ be square? Must $\left(X^T X\right)$ be square? Must $X$ be square?
\item[(b)] Show that matrix $A$ is idempotent.
\end{enumerate}
\end{itemize}

%%%%% Exercise 5.1

\underline{Exercise 5.1}
\begin{enumerate}
% Question 3
\item[3.] Are the rows linearly independent in each of the following?
\begin{multicols}{2}
\begin{enumerate}
\item[(a)] $\left[\begin{array}{rr}24 & 8 \\ 9 & -3\end{array}\right]$
\item[(b)] $\left[\begin{array}{ll}2 & 0 \\ 0 & 2\end{array}\right]$
\item[(c)] $\left[\begin{array}{ll}0 & 4 \\ 3 & 2\end{array}\right]$
\item[(d)] $\left[\begin{array}{rr}-1 & 5 \\ 2 & -10\end{array}\right]$
\end{enumerate}
\end{multicols}

% Question 4
\item[4.] Check whether the columns of each matrix in Prob. 3 are also linearly independent. Do you get the same answer as for row independence?
\end{enumerate}
