...
:::
::: {.column width="40%"}
![](../../assets/fig/stem-linal.png){fig-alt="Comic showing a precarious tower of blocks labeled 'All of STEM' balancing on one small block labeled 'That one linear algebra course'" width=300}
:::
:::
...
Denote $$Y  = \begin{bmatrix} Y_1 \\ Y_2 \\  \vdots \\ Y_n  \end{bmatrix}_{n \times 1},    
  ...
  \varepsilon  = \begin{bmatrix} \varepsilon_1 \\ \varepsilon_2 \\ \vdots \\ \varepsilon_n  \end{bmatrix}_{n \times 1}$$ Then, $$Y = X \beta + \varepsilon  \quad \quad \text{OLS estimator: } \hat{\beta} = (X^T X)^{-1} X^T Y$$
...
Sample exams and a help sheet are posted on the Exams page

Notes for reviewing Linear Algebra are on the website (not a substitute for the textbook for understanding concepts)

After the midterm, we move on to differential calculus

## Homework Questions

-   Exercise 5.4: 2, 3, 4, 6, 7
